...
class Validate {
    ...
    public function birthdate($name, $value, $required = true)
    {
        ...
        // Validate against a pattern, if it does not match throw error
        $pattern = '/^(0?[1-9]|1[0-2])\/([0-2][0-9]|3[0-1])\/\d{4}$/';
        $message = 'Invalid format';
        $this->pattern($name, $value, $pattern, $message, $required);
        
        //test birthday is not future date
        $birthday = new \DateTime($value);
        $now = new \DateTime();
        if ($birthday > $now)
        {
            $field->setErrorMessage('Birth date can not be in the future.');
            return;
        }
    }
    ...
    public function email($name, $value, $required = true) {
        ...
        if($email === false)
        {
            $field->setErrorMessage('Invalid email.');
        }
        else
        {
            $field->clearErrorMessage();
...
//                'Invalid domain name part.');
    ...
    public function password($name, $password, $required = true) {
        ...
        if ($pwMatch === false) {
            ...
            $field->setErrorMessage(
                    'Must have one each of uppercase letter and digit.');
            return;
        }
    }
    ...
    public function verify($name, $password, $verify, $required = true) {
        ...
        //Noted: number above after required is LENGTH of password needed
        if ($field->hasError()) { return; }
